<script lang="ts">
	...
	const fisrt_name_letters = slice.primary.first_name?.split('') ?? '';
...
>
	<div class="mx-auto w-full max-w-7xl">
		<div class="grid min-h-[65vh] items-center grid-cols-1 md:grid-cols-2">
			<div
				class="relative z-10 row-span-1 row-start-1 -my-10 aspect-[1/1.3]
				overfow-hidden md:col-span-1 md:col-start-2 md:mt-0"
			>
				<Scene />
			</div>
		...
		tracking-tighter text-nowrap"
				...
				>
					{#if fisrt_name_letters.length && last_name_letters.length}
						<span class="block text-slate-300">
							{#each fisrt_name_letters as letter}
		...
		bg-clip-text text-transparent text-2xl font-bold uppercase tracking-[0.2em]
		md:text-4xl opacity-0"
				>
