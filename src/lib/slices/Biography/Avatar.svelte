<script lang="ts">
	...
	onMount(() => {
		...
		window.onmousemove = (e) => {
			...
			// How far away the mouse is from the center of the component
			...
			gsap
				...
				.to(
					'.avatar',
					{
						rotation: gsap.utils.clamp(-2, 2, componentPercent.x * 5)
					},
					0
				)
				.to(
...
<div class={clsx(`relative h-full w-full`, className)} bind:this={component}>
	<div
		class="avatar aspect-square overflow-hidden rounded-3xl boder-2
	...
	>
		...
		/>
		<div
			class="highlight absolute inset-0 w-full scale-110
        bg-gradient-to-tr from-transparent via-white to-transparent opacity-0"
		></div>
	</div>
</div>
