...
{#if type === 'desktop'}
    ...
    px-3 py-1 text-base font-bold text-slate-900"
		{field}
	...
	>
      ...
      transiiton-transform duration-300 ease-in-out  group-hover:translate-y-0 
...
{:else}
	<PrismicLink
		class="group relative block overflow-hidden rounded
    px-3 text-3xl font-bold text-slate-900"
		{field}
		on:click={onLinkClick}
		aria-current={isActive ? 'page' : undefined}
	>
